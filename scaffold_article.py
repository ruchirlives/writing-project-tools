...
def scaffold(target_dir: Path, force: bool, install: bool, use_uv: bool) -> None:
    target_dir.mkdir(parents=True, exist_ok=True)

    created: list[str] = []
    skipped: list[str] = []

    template = TOOLKIT_DIR / "AGENTS.template.md"
    agents_target = target_dir / "AGENTS.md"
    if template.exists():
        if agents_target.exists() and not force:
            skipped.append("AGENTS.md")
        else:
            agents_target.write_text(template.read_text(encoding="utf-8"), encoding="utf-8")
    ...
    readme = (
        ...
        "```powershell\n"
        "uv venv\n"
        f"uv pip install -e \"{TOOLKIT_DIR}\"\n"
        "```\n\n"
        "## Assertions Editor\n\n"
        "```powershell\n"
        "uv run edit-assertions\n"
        "```\n\n"
        "## Generate Word Documents\n\n"
        "```powershell\n"
        "uv run create-writing-docs\n"
        "```\n"
    )
    ...
    print(f"Scaffolded writing project at {target_dir}")
    if created:
...
def main() -> None:
    args = parse_args()
    scaffold(Path(args.target).resolve(), args.force, args.install, not args.no_uv)
